...
class RawFrameClsDataset(Dataset):
    ...
    def load_frame(self, sample, num_frames, sample_rate_scale=1):
        """Load video frames based on directory order, ignoring filename numerics."""
        # List all .png files in the sample directory, sorted by their natural order
        image_files = [f for f in sorted(os.listdir(sample)) if f.endswith('.png')]
    
        if self.mode == 'test':
            # Calculate frame indices for test mode
            tick = len(image_files) / float(self.test_num_segment)
            all_index = []
            for t_seg in range(self.test_num_segment):
                start_index = int(tick * t_seg)
                end_index = int(tick * (t_seg + 1))
                segment_indices = np.linspace(start_index, end_index, self.num_segment, endpoint=False, dtype=int)
                all_index.extend(segment_indices)
        else:
            # Calculate frame indices for train/validation mode
            average_duration = len(image_files) // self.num_segment
            all_index = []
            if average_duration > 0:
                offsets = np.multiply(list(range(self.num_segment)), average_duration) + average_duration // 2
            elif len(image_files) > self.num_segment:
                offsets = np.linspace(0, len(image_files) - 1, self.num_segment, dtype=int)
            else:
                offsets = np.arange(self.num_segment) % len(image_files)
            all_index.extend(offsets)
    
        # Load the frames based on calculated indices
        imgs = []
        for idx in all_index:
            if idx < len(image_files):
                frame_fname = os.path.join(sample, image_files[idx])
                try:
                    img = self.image_loader(frame_fname)  # Assuming this method loads an image file into a numpy array
                    imgs.append(img)
                except IOError as e:
                    print(f"Error loading image: {frame_fname} - {e}")
            else:
                print(f"Index {idx} is out of bounds for available images.")
                
        # Debugging: Check if imgs list is empty
        if not imgs:
            print("No images were loaded. Check file paths and image loading logic.")
            return np.array([])  # Return an empty array if no images are loaded
    
        # Optional: Verify that all images have the same shape
        first_img_shape = imgs[0].shape
        if not all(img.shape == first_img_shape for img in imgs):
            print("Images have inconsistent shapes. Ensure all images are the same size and format.")
            return np.array([])  # Return an empty array if images have inconsistent shapes
    
    
        buffer = np.stack(imgs, axis=0) if imgs else np.array([])  # Stack images into a single array
